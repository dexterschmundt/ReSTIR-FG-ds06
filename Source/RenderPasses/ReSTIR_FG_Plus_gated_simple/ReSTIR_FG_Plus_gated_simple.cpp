#include "ReSTIR_FG_Plus_gated_simple.h"
#include <memory>
#include <utility>
...
namespace
{
const std::string kShaderFolder = "RenderPasses/ReSTIR_FG_Plus_gated_simple/";
...
{
    registry.registerClass<RenderPass, ReSTIR_FG_Plus_gated_simple>();
}

ReSTIR_FG_Plus_gated_simple::ReSTIR_FG_Plus_gated_simple(ref<Device> pDevice, const Properties& props) : RenderPass(pDevice)
{
...
Properties ReSTIR_FG_Plus_gated_simple::getProperties() const
{
    return {}; // TODO
}

RenderPassReflection ReSTIR_FG_Plus_gated_simple::reflect(const CompileData& compileData)
{
...
void ReSTIR_FG_Plus_gated_simple::renderUI(Gui::Widgets& widget)
{
    bool changed = false;
    ...
    if (auto group = widget.group("Photon Options"))
    {
        ...
        group.tooltip(
            "Percentage the CPU photon count value (which is delayed by 1-3 frames) is overestimated to improve acceleration structure "
            "build time."
        );
        ...
        group.tooltip(
            "Enables adaptive photon radius, that is equal to the projected camera pixel size, which depends on the distance from the "
            "camera to the hit point"
        );
    ...
    if (auto group = widget.group("ReSTIR-FG+"))
    {
        ...
        group.tooltip("Maximum Path length for a initial path sample. A path sample stops, when it encounters a diffuse surface");
        ...
        group.tooltip(
            "If enabled photon shift is performed. On static scenes, photon shift can always be assumed true and therefore does not need "
            "to be performed."
        );
    ...
    if (auto group = widget.group("Material Options"))
    {
        ...
        group.tooltip(
            "Determines the diffuse surface classification: \n"
        ...
        group.tooltip(
            "Used to determine if a reconnection can be performed. Additionally if the classification is choosen, it is also used to "
            "determine if a surface is diffuse"
        );
        group.indent(10.f);
...
void ReSTIR_FG_Plus_gated_simple::setScene(RenderContext* pRenderContext, const ref<Scene>& pScene)
{
    // Reset Scene
    mpScene = pScene;
...
void ReSTIR_FG_Plus_gated_simple::execute(RenderContext* pRenderContext, const RenderData& renderData)
{
    if (!mpScene)
        return;
...
void ReSTIR_FG_Plus_gated_simple::resetAllRenderPasses()
{
...
void ReSTIR_FG_Plus_gated_simple::prepareLightingStructure(RenderContext* pRenderContext)
{
...
void ReSTIR_FG_Plus_gated_simple::prepareResources(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    if (!mpPhotonHitInfo)
    {
        mpPhotonHitInfo = Buffer::createStructured(
            mpDevice, sizeof(uint) * 4, mOptions.photonBufferSizeCaustic,
            ResourceBindFlags::ShaderResource | ResourceBindFlags::UnorderedAccess, Buffer::CpuAccess::None, nullptr, false
        );
...
void ReSTIR_FG_Plus_gated_simple::tracePhotonsPass(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    uint2 currentPhotons = mFrameCount > 0 ? uint2(float2(mPhotonCountUI) * mOptions.photonASBuildBufferOverestimate)
...
void ReSTIR_FG_Plus_gated_simple::shiftCausticPathPass(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    // Dispatch raytracing shader
    mpScene->raytrace(
        pRenderContext, mShiftCausticPathPass.pProgram.get(), mShiftCausticPathPass.pVars, uint3(mScreenRes.x, mScreenRes.y, 1)
    );
}
